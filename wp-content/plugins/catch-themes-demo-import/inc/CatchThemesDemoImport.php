<?php
/**
 * Main Catch Themes Demo Import plugin class/file.
 *
 * @package ctdi
 */
...
class CatchThemesDemoImport {
	...
	/**
	 * The instance of the CTDI\Importer class.
	 *
	 * @var object
	 */
	public $importer;
  ...
  private $plugin_page_setup = array();
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'cp-ctdi/plugin_page_setup', array(
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ctdi-main-js', 'ctdi',
				array(
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $this->before_import_executed ) {
			...
			/**
			 * 2). Execute the actions hooked to the 'cp-ctdi/before_content_import_execution' action:
			 *
			 * Default actions:
		...
		/**
		 * 4). Execute the actions hooked to the 'cp-ctdi/after_content_import_execution' action:
		 *
		 * Default actions:
	...
	public function setup_plugin_with_filter_data() {
		 ...
		 * to the 'before_content_import_execution' and the 'cp-ctdi/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = apply_filters( 'cp-ctdi/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'cp-ctdi/logger_options', array(
	...
	/**
	 * Catch Theme Demo Import: action_links
	 * Catch Theme Demo Import Settings Link function callback
	 *
